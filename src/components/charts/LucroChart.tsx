...
import { getTransformedChartDataFromPercentages } from "../../utils/dataUtil";
...
export default function LucroChart(props: any) {
  const { timeContext } = useChartInfo();
  const { empresa } = useApp();

  const { isLoading, data, error } = useQuery({
    queryKey: ["lucroData", timeContext, empresa, props.banco],
    queryFn: async () =>
        ...
        <>
          ...
          <div className='w-[350px] h-[250px] flex justify-center items-center'>
            <GenericBarChart
              ...
              data={getTransformedChartDataFromPercentages(chartData)}
            />
          </div>
          {isLoading && (
