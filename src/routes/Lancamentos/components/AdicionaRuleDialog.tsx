...
import { z } from "zod";
import { CalendarIcon, CheckIcon, PlusIcon, X } from "lucide-react";
...
import { RuleDTO } from "@/utils/types";
import {
  dateCondtList,
  fieldList,
  genericCondition,
...
const adicionaformSchema = z.object({
  nome: z
  ...
  categoria: z.string({ required_error: "categoria é obrigatório" }),
});

type AdicionaRuleDialogProps = PropsWithChildren<{
  handleSubmit: (rule: RuleDTO) => Promise<void>;
  categorias: Array<any>;
}>;

export default function AdicionaRuleDialog({
  handleSubmit,
  categorias,
  children,
}: AdicionaRuleDialogProps) {
  const defaultRule: RuleDTO = {
    nome: "",
  ...
  const form = useForm<z.infer<typeof adicionaformSchema>>({
    resolver: zodResolver(adicionaformSchema),
  ...
  async function onSubmitForm(values: z.infer<typeof adicionaformSchema>) {
    await handleSubmit(values as RuleDTO);
                                ...
                                {field.campo === "data" && (
                                  <Popover>
                                    ...
                                    <PopoverContent className="w-auto p-0">
                                      <Calendar
                                        mode="single"
                                        selected={field?.valor}
                                        onSelect={(day) => {
                                          if (
                                            day &&
                                            !isEqual(day, field?.valor)
                                          ) {
                                            update(index, {
                                              ...field,
                                              valor: day,
