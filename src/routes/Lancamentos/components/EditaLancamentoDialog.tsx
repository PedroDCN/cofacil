...
export default function EditaLancamentoDialog({
  selectedLancamento,
  categorias,
  bancos,
  handleSubmit,
    ...
    <>
      <Dialog>
        <DialogTrigger
          asChild
          onClick={() => {
            form.setValue("valor", String(selectedLancamento.valor));
            form.setValue("tipo", selectedLancamento.tipo);
            // TODO: bug na conversão de data devido ao formato de data recebido do servidor,
            // por isso necessário adicionar ':' ao final para incluir as 'horas' na data
            // e a conversão ocorrer corretamente
            form.setValue("data", new Date(selectedLancamento.data + ":"));
            form.setValue("descricao", selectedLancamento.descricao || "");
            form.setValue("comentarios", selectedLancamento.comentarios || "");
            const categoria = categorias.find(
              (categoria: any) =>
                categoria.nome === selectedLancamento.nomeCategoria
            );
            const banco = bancos.find(
              (banco: any) => banco.nome === selectedLancamento.nomeBanco
            );
            form.setValue("idCategoria", categoria.id);
            form.setValue("bancoId", banco.id);
          }}
        >
          {children ? (
            children
            ...
            <Button className="bg-green-500 hover:bg-green-700 rounded-sm text-md">
              <i className="pi pi-plus mr-2"></i>Editar
            </Button>
          )}
        </DialogTrigger>
        <DialogContent className="min-w-[500px] sm:max-w-md">
          <DialogHeader>
            <DialogTitle>Edição de Lançamento</DialogTitle>
          </DialogHeader>
          <Form {...form}>
            <form
